...
y_hat <- linear(x) # filter for the linear regression line

blue_pt <- background %>% filter(Var2 < y_hat) 
orange_pt <- background %>% filter(Var2 > y_hat)

# Figure
plot(blue_obs, col='deepskyblue', xlim = c(-1.5,4), ylim = c(-2,3.5), 
     xlab='', ylab='', lwd=3, cex = 1.5, xaxt='n', yaxt='n')
points(orange_obs, col='orange', lwd = 3, cex = 1.5)
